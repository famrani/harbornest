...
export class AdminMediaService {
  ...
  async upload(file: File, category: 'outings' | 'boat' | 'gallery' | 'content', subject: string): Promise<AdminMediaUploadResult> {
    ...
    data.append('assetType', this.resourceContext.assetType);
    data.append('assetId', this.resourceContext.assetId);
  ...
  previewUrl(path: string): string {
    ...
    if (value.startsWith('assets/img/')) {
      const canonical = `${this.resourceContext.assetType}/${this.resourceContext.assetId}/img/${value.slice('assets/img/'.length)}`;
      return `/api/media/object?path=${encodeURIComponent(canonical)}`;
    }
    if (value.startsWith(`${this.resourceContext.assetType}/${this.resourceContext.assetId}/img/`)) {
