...
type CmsTab = 'home' | 'outings' | 'boat' | 'pricing' | 'contact';
...
export class AdminSiteContentComponent implements OnInit {
  ...
  constructor(
    ...
    this.boatId = this.resourceContext.boatId;
  }
  ...
  get homePage(): any {
    ...
    this.current.homePage.hero = this.current.homePage.hero || {};
    this.current.homePage.experiences = this.current.homePage.experiences || {};
    this.current.homePage.boat = this.current.homePage.boat || {};
    this.current.homePage.journey = this.current.homePage.journey || {};
    this.current.homePage.finalCta = this.current.homePage.finalCta || {};
    ...
    try {
      const [site, operationalPricing, boats] = await Promise.all([
        firstValueFrom(this.http.get<any>(
          `${this.firebaseDatabaseUrl}/${this.resourceContext.scopedPath('siteContent')}.json`,
          this.rawFirebaseOptions,
        ).pipe(timeout(15000))),
        firstValueFrom(this.http.get<any>(`${this.firebaseDatabaseUrl}/${this.resourceContext.scopedPath('bnPricingModel')}.json`).pipe(timeout(15000))),
  ...
  async selectBoat(boatId: string): Promise<void> {
    if (boatId !== this.boatId) {
      this.resourceContext.setResourceId(boatId);
      return; // The new URL reloads every service for the selected boat.
    }
    await this.load();
  }
    ...
    try {
      ...
      for (const language of this.languages) {
        ...
        for (const section of sections) {
          updates[section] = this.siteContent?.[language.id]?.[section] ?? null;
        }
        writes.push(firstValueFrom(
          this.http.patch(
            `${this.firebaseDatabaseUrl}/${this.resourceContext.scopedPath('siteContent', language.id)}.json`,
            updates
          ).pipe(timeout(15000))
        ));
      }
      ...
      if (this.activeTab === 'pricing') {
        writes.push(firstValueFrom(
          this.http.put(`${this.firebaseDatabaseUrl}/${this.resourceContext.scopedPath('bnPricingModel')}.json`, this.pricing)
            .pipe(timeout(15000))
        ));
      }
  ...
  private getTabPayload(language: LanguageCode): any {
    ...
    switch (this.activeTab) {
      case 'home': return { homePage: lang.homePage || {} };
  ...
  private async saveOperationalOutings(): Promise<any> {
    ...
    try {
      existing = await firstValueFrom(this.http.get<any>(
        `${this.firebaseDatabaseUrl}/${this.resourceContext.scopedPath('bnOutings')}.json`,
        this.rawFirebaseOptions,
      ).pipe(timeout(15000))) || {};
    } catch {}
  ...
  private localizedOutingPayload(outing: any): any {
    const keys = [
      'title', 'description', 'duration', 'guests', 'priceLabel', 'priceDetails', 'highlights',
