...
export class AdminExternalBookingsComponent implements OnInit, OnDestroy {
  ...
  openBooking(booking: AlegriaBooking): void {
    ...
    if (!id) return;
    window.location.href = boatPath(`/admin/reservations/${encodeURIComponent(id)}`);
  }
  ...
  async saveExternalBooking(): Promise<void> {
    ...
    try {
      ...
      this.loadBoats();
      this.message = `${this.t('savedMessage')} ${window.location.origin}${boatPath("/offer")}/${saved.offerId}`;
    } catch (e: any) {
      this.error = e?.message || this.t('saveError');
    }
