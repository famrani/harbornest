...
function proposalFinancialBreakdown(record: any) {
...
export class BookingsService {
    ...
    private async notifyBookingRequestCreated(req: Request, proposalId: string): Promise<void> {
        const snap = await this.storeDbc.db.ref(`/bnProposals/${proposalId}`).once('value');
        const proposal = snap.val();
        if (!proposal) throw new Error('Proposal request not found');
    ...
    async setRoutes(router) {
        ...
        router.post('/api/proposals/:proposalId/notify-request-created', async (req: Request, res: Response) => {
            try {
