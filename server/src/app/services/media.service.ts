...
export class MediaService {
  ...
  private readonly supportedResourceTypes = new Set(['boat', 'place', 'car', 'pool']);
  ...
  setRoutes(router: Router): void {
    const limiter = rateLimit({
      ...
      legacyHeaders: false,
      // Local development often reloads many images at once. Do not let those
      // requests exhaust the public media limiter. This bypass is deliberately
      // disabled in production so a forged Host header cannot disable limits.
      skip: (req: Request) => {
        if (process.env.NODE_ENV === 'production') return false;
        return this.isLocalHostname(this.getRequestHostname(req));
      },
    });
  ...
  private async uploadObject(req: Request, res: Response): Promise<Response> {
    try {
      ...
      // Resource-scoped storage layout (Release 112):
      //   <resourceType>/<resourceId>/img/<category>/<subject>/<file>
      ...
      const resourceType = this.safeResourceType(
        req.body?.resourceType || req.body?.assetType || 'boat',
      );
      const resourceId = this.safeSegment(
        req.body?.resourceId || req.body?.assetId || req.body?.boatId || tenant.id,
      );
  ...
  private safeResourceType(value: unknown): string {
    const resourceType = this.safeSegment(value);
    if (!this.supportedResourceTypes.has(resourceType)) throw new Error('invalid_identifier');
    return resourceType;
  }
  ...
  private isAllowedPath(objectPath: string, tenant: ResolvedStorageTenant): boolean {
    ...
    const structurallySafe = path.length <= 512
      && !path.includes('..')
      && !path.includes('\\')
      && /^[a-zA-Z0-9._/\-]+$/.test(path);
    if (!structurallySafe) return false;

    // Legacy Alegria objects remain readable during migration.
    if (tenant.mediaPrefix && path.startsWith(tenant.mediaPrefix)) return true;

    // New resource-scoped objects are accepted for every supported resource
    // type. The tenant root still comes exclusively from trusted backend config.
    const match = /^(boat|place|car|pool)\/([a-z0-9][a-z0-9_-]{0,79})\/img\//.exec(path);
    return !!match && this.supportedResourceTypes.has(match[1]);
  }
  ...
  private async loadTenantConfigs(): Promise<Record<string, StorageTenantConfig>> {
    ...
    try {
      ...
      // Release 114: accept both supported Firebase layouts during the
      // resource migration:
      //   /storageTenants/alegria
      //   /storageTenants/boat/alegria
      // The media service itself still resolves a tenant by tenant id.
      values = this.normalizeTenantConfigs(raw);
    } catch (error) {
  ...
  private normalizeTenantConfigs(raw: any): Record<string, StorageTenantConfig> {
    const normalized: Record<string, StorageTenantConfig> = {};
    if (!raw || typeof raw !== 'object') return normalized;

    const looksLikeTenant = (value: any) => !!value && typeof value === 'object'
      && (typeof value.bucket === 'string' || typeof value.root === 'string' || typeof value.mediaPrefix === 'string');

    // Legacy/current layout: /storageTenants/<tenantId>
    Object.entries(raw).forEach(([id, value]) => {
      if (looksLikeTenant(value)) normalized[id] = value as StorageTenantConfig;
    });

    // Resource-scoped layout introduced by the DB migration:
    // /storageTenants/<resourceType>/<resourceId>.  Flatten it for tenant
    // resolution while preserving direct entries above as the authority.
    for (const resourceType of this.supportedResourceTypes) {
      const scoped = raw[resourceType];
      if (!scoped || typeof scoped !== 'object') continue;
      Object.entries(scoped).forEach(([id, value]) => {
        if (!normalized[id] && looksLikeTenant(value)) {
          normalized[id] = value as StorageTenantConfig;
        }
      });
    }
