...
class BoatownersService {
    ...
    setRoutes(router) {
        ...
        router.post('/boatowners/config/xls/upload', upload.single('file'), async (req, res) => {
            try {
                const ownerId = req.body?.ownerId;
                const file = req.file; // ✔️ Works with modern TS + multer
                if (!ownerId || !file) {
        ...
        router.post('/boatowners/config/xls/import', async (req, res) => {
            ...
            try {
                ...
                const homepageRows = xlsx.utils.sheet_to_json(homepageSheet, { defval: '' });
                const eventsRows = eventsSheet ? xlsx.utils.sheet_to_json(eventsSheet, { defval: '' }) : [];
                const boatsRows = boatsSheet ? xlsx.utils.sheet_to_json(boatsSheet, { defval: '' }) : [];
                const skipperRows = skipperSheet ? xlsx.utils.sheet_to_json(skipperSheet, { defval: '' }) : [];
                ...
                const mainpageId = hp.mainpageId;
                if (!mainpageId) {
...
exports.BoatownersService = BoatownersService;

//# sourceMappingURL=boatowners.service.js.map
