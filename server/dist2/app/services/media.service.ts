...
export class MediaService {
  ...
  private async uploadObject(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(201).json({
        ok: true,
        path: logicalPath,
        url: `/api/media/object?path=${encodeURIComponent(logicalPath)}&boatId=${encodeURIComponent(tenant.id)}`,
        contentType: file.mimetype,
        size: file.size,
      });
    } catch (error: any) {
  ...
  private async createMediaUrls(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const entries: Array<readonly [string, string]> = paths.map((objectPath) => [
        objectPath,
        `/api/media/object?path=${encodeURIComponent(objectPath)}&boatId=${encodeURIComponent(tenant.id)}`,
      ] as const);
  ...
  private async resolveTenant(req: Request): Promise<ResolvedStorageTenant | null> {
    ...
    // A shared HarborDirect hostname hosts several boats. The client must name
    // the boat for uploads and may name it for reads. Configuration remains
    // authoritative: callers can never provide a bucket or physical root.
    const objectPathTenant = String(req.query?.path || req.body?.path || '').split('/')[0];
    const requestedTenant = String(
      req.body?.boatId || req.query?.boatId || req.headers['x-boat-id'] || objectPathTenant || '',
    ).trim().toLowerCase();
    if (requestedTenant && !/^[a-z0-9][a-z0-9_-]{0,79}$/.test(requestedTenant)) return null;

    if (requestedTenant && (!values[requestedTenant] || values[requestedTenant]?.enabled === false)) return null;
