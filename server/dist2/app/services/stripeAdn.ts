...
// The only Stripe secret used by the application. Every boat owner is a
// connected account selected through Stripe's `stripeAccount` context.
const PLATFORM = new Stripe(process.env.STRIPE_SECRET_KEY as string, {
    apiVersion: STRIPE_API_VERSION,
});

/**
 * Resolve a Stripe client scoped to an owner's connected account. No owner
 * secret key or OAuth token is stored by HarborDirect.
 */
async function getOwnerStripe(db: any, ownerId: string): Promise<Stripe> {
    const ownerSnap = await db.ref(`/backendowners/${ownerId}/stripeConnect`).once('value');
    const userSnap = ownerSnap.exists() ? null : await db.ref(`/backendusers/${ownerId}/stripeConnect`).once('value');
    const data = ownerSnap.val() || userSnap?.val();
    const accountId = String(data?.accountId || '').trim();
    if (!accountId || data?.chargesEnabled !== true) {
        throw new Error('Owner Stripe account is not connected or cannot accept charges.');
    }
    // Stripe-Account is a per-request option, not another API key. Wrap the
    // platform client so all existing payment code automatically supplies it.
        ...
        get(object: any, property: string | symbol) {
            ...
            if (typeof value === 'function') {
                return (...args: any[]) => {
                    ...
                    if (args.length === 0) return value.call(object, undefined, options);
                    if (args.length === 1 && (typeof args[0] === 'string' || typeof args[0] === 'number')) {
                        return value.call(object, args[0], undefined, options);
                    }
                    return value.call(object, ...args, options);
                };
            }
            return value && typeof value === 'object' ? wrap(value) : value;
        }
    });
    return wrap(PLATFORM) as Stripe;
}
...
export class StripeService {
    ...
    /** Create/reuse a Standard connected account and redirect to onboarding. */
    async connectAuthorize(req: Request, res: Response) {
        try {
            const { ownerId, returnUrl, refreshUrl, email, country = 'FR' } = req.query as {
                ownerId?: string;
                returnUrl?: string;
                refreshUrl?: string;
                email?: string;
            ...
            const path = `/backendowners/${ownerId}/stripeConnect`;
            const existing = await this.stbDbSvc.db.ref(path).once('value').then(s => s.val()).catch(() => null);
            let accountId = String(existing?.accountId || '');
            if (!accountId) {
                const account = await PLATFORM.accounts.create({
                    type: 'standard',
                    country: String(country || 'FR').toUpperCase(),
                    email: email || undefined,
                    metadata: { ownerId, platform: 'HarborDirect' },
                });
                ...
                await this.stbDbSvc.db.ref(path).set({
                    ...
                    livemode: !account.id.startsWith('acct_') ? false : process.env.STRIPE_SECRET_KEY?.startsWith('sk_live_') === true,
                    connectedAt: Date.now(),
                    modifiedAt: Date.now(),
                });
            }
            const fallbackReturn = process.env.STRIPE_CONNECT_RETURN_URL || `${process.env.PUBLIC_APP_URL || ''}/${ownerId}/admin/fleet`;
            const fallbackRefresh = process.env.STRIPE_CONNECT_REFRESH_URL || fallbackReturn;
            const link = await PLATFORM.accountLinks.create({
                account: accountId,
                type: 'account_onboarding',
                return_url: returnUrl || fallbackReturn,
                refresh_url: refreshUrl || fallbackRefresh,
            });
    ...
    /** Compatibility callback: onboarding returns directly to the frontend. */
    async connectCallback(req: Request, res: Response) {
        try {
            res.redirect(process.env.STRIPE_CONNECT_RETURN_URL || process.env.PUBLIC_APP_URL || '/');
        } catch (err: any) {
    ...
    /** Disable the local connection. The Stripe account itself is never deleted. */
    async connectDeauthorize(req: Request, res: Response) {
        try {
            const { ownerId } = req.body as { ownerId?: string };
            if (!ownerId) return res.status(400).json({ error: 'ownerId required' });

            await this.stbDbSvc.db.ref(`/backendowners/${ownerId}/stripeConnect`).update({
    ...
    async checkoutSetup(req: Request, res: Response) {
        try {
            ...
            const { boatId } = await this.resolvePaymentContext(bookingId, ownerId);
            ...
            const session = await stripe.checkout.sessions.create({
                ...
                metadata: { bookingId, boatId },
            });

            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payment`).update({
                status: 'init',
    ...
    async acceptAndCharge(req: Request, res: Response) {
        try {
            ...
            const bookingSnap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value');
            ...
            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                status: 'confirmed',
                updatedAt: Date.now(),
            });
            ...
            const { bookingId, ownerId } = (req.body || {}) as any;

            if (bookingId) {
                const failedContext = await this.resolvePaymentContext(bookingId, ownerId).catch(() => null);
                const failedBoatId = failedContext?.boatId || String((req.body || {}).boatId || 'alegria');
                await this.stbDbSvc.db.ref(`/bnBookings/${failedBoatId}/${bookingId}/payment`).update({
                    status: 'charge_failed',
                    lastError: e?.message || 'Charge failed',
                    updatedAt: Date.now(),
                });
                await this.stbDbSvc.db.ref(`/bnBookings/${failedBoatId}/${bookingId}`).update({
                    status: 'pending',
                    updatedAt: Date.now(),
                });
            }
    ...
    private async assertTermsAcceptedBeforeCustomerAction(bookingId: string): Promise<any> {
        ...
        const { boatId } = await this.resolvePaymentContext(id);
        ...
        const directBookingSnap = await db.ref(`/bnBookings/${boatId}/${id}`).once('value');
        addCandidate(directBookingSnap.val(), `/bnBookings/${boatId}/${id}`);
        ...
        for (const field of bookingLookupFields) {
            try {
                const snap = await db.ref(`/bnBookings/${boatId}`).orderByChild(field).equalTo(id).once('value');
                const matches = snap.val() || {};
                Object.keys(matches).forEach((key) => addCandidate(matches[key], `/bnBookings/${boatId}/${key} via ${field}`));
            } catch (queryError) {
        ...
        for (const offerId of relatedOfferIds) {
            const directOfferSnap = await db.ref(`/bnProposals/${boatId}/${offerId}`).once('value');
            addOffer(directOfferSnap.val(), `/bnProposals/${boatId}/${offerId}`);
        }
        ...
        for (const lookupId of relatedOfferIds) {
            for (const field of offerLookupFields) {
                try {
                    const snap = await db.ref(`/bnProposals/${boatId}`).orderByChild(field).equalTo(lookupId).once('value');
                    const matches = snap.val() || {};
                    Object.keys(matches).forEach((key) => addOffer(matches[key], `/bnProposals/${boatId}/${key} via ${field}`));
                } catch (queryError) {
    ...
    /**
     * Keep the strict persisted-acceptance guard, while allowing a checkout
     * request that carries an explicit acceptance action to create the missing
     * audit record. This is required for imported/external bookings whose offer
     * was accepted before the canonical booking node was created.
     */
    private async assertOrPersistCheckoutTermsAcceptance(bookingId: string, body: any): Promise<any> {
        try {
            return await this.assertTermsAcceptedBeforeCustomerAction(bookingId);
        } catch (termsError) {
            if (!this.hasExplicitCustomerTermsAcceptance(body)) throw termsError;

            const context = await this.resolvePaymentContext(bookingId, body?.ownerId);
            const booking = context.booking || {};
            if (!Object.keys(booking).length) throw termsError;

            const acceptedAt = body?.termsAcceptedAt || body?.tncAcceptedAt || Date.now();
            const acceptedBy = body?.termsAcceptedBy || body?.tncAcceptedBy || body?.customerUserId || body?.userId || 'customer';
            const now = Date.now();
            const auditId = `acceptance_${now}`;
            const requestIp = String(body?.customerIp || '').trim();
            const source = 'checkout_request';
            const patch = {
                termsAccepted: true,
                tncAccepted: true,
                ...
                terms: { ...(booking.terms || {}), accepted: true, acceptedAt, acceptedBy, source },
                workflow: { ...(booking.workflow || {}), termsAccepted: true, termsAcceptedAt: acceptedAt, termsAcceptedBy: acceptedBy, termsAcceptedSource: source },
                bookingWorkflow: { ...(booking.bookingWorkflow || {}), termsAccepted: true, termsAcceptedAt: acceptedAt, termsAcceptedBy: acceptedBy, termsAcceptedSource: source },
                modifiedTS: now,
                updatedAt: now,
            };
            const audit = {
                ...
                acceptedBy,
                source,
                termsVersion: body?.termsVersion || booking?.termsVersion || 'current',
                customerIp: requestIp || null,
                userAgent: body?.userAgent || null,
                createdTS: now,
            };
            const updates: Record<string, any> = {};
            updates[`/bnBookings/${context.boatId}/${bookingId}`] = { ...booking, ...patch };
            updates[`/bnBookings/${context.boatId}/${bookingId}/termsAcceptanceHistory/${auditId}`] = audit;
            await this.stbDbSvc.db.ref().update(updates);
            return { ...booking, ...patch };
        }
    }

    async acceptOutingTerms(req: Request, res: Response) {
        try {
            const body: any = req.body || {};
            const bookingId = String(body.bookingId || body.offerId || req.params?.bookingId || '').trim();
            if (!bookingId) return res.status(400).json({ error: 'bookingId is required' });
            if (!this.hasExplicitCustomerTermsAcceptance(body)) {
                return res.status(400).json({ error: 'Explicit Terms & Conditions acceptance is required' });
            }
            const accepted = await this.assertOrPersistCheckoutTermsAcceptance(bookingId, body);
            const context = await this.resolvePaymentContext(bookingId, body.ownerId);
            return res.json({
                ok: true,
                bookingId,
                boatId: context.boatId,
                ownerId: context.ownerId,
                termsAccepted: this.hasExplicitCustomerTermsAcceptance(accepted),
                termsAcceptedAt: accepted.termsAcceptedAt || accepted.tncAcceptedAt || null,
            });
        } catch (e: any) {
            console.error('[acceptOutingTerms] error:', e);
            return res.status(400).json({ error: e?.message || 'Unable to record Terms & Conditions acceptance' });
        }
    }
    ...
    private async assertBalanceCheckoutAllowed(bookingId: string, body: any): Promise<void> {
        ...
        try {
            ...
            if (!this.hasExplicitCustomerTermsAcceptance(body)) throw termsError;
            const existingSnap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value');
            const existing = existingSnap.val();
            if (!existing) throw termsError;
            const acceptedAt = body.termsAcceptedAt || body.tncAcceptedAt || Date.now();
            const termsPatch = {
                termsAccepted: true,
                tncAccepted: true,
            ...
            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update(termsPatch);
            acceptedEntity = { ...existing, ...termsPatch };
        }

        const directBookingSnap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value');
    ...
    private buildBookingPaymentPath(boatId: string, bookingId: string, child?: string): string {
        return child
            ? `/bnBookings/${boatId}/${bookingId}/payments/${child}`
            : `/bnBookings/${boatId}/${bookingId}/payments`;
    }
    ...
    private async getStripeForOwner(ownerId: string): Promise<Stripe> {
        if (ownerId === 'platform') return PLATFORM;
     ...
     */
    private async resolvePaymentContext(referenceId: string, requestedOwnerId?: string): Promise<{
        bookingId: string;
        boatId: string;
        ownerId: string;
        booking: any;
        ...
        const [bookingsRoot, proposalsRoot] = await Promise.all([
            this.stbDbSvc.db.ref('/bnBookings').once('value').then(s => s.val()).catch(() => ({})),
            this.stbDbSvc.db.ref('/bnProposals').once('value').then(s => s.val()).catch(() => ({})),
        ]);
        let booking: any = {};
        let proposal: any = {};
        let locatedBoatId = '';
        for (const candidateBoatId of new Set([...Object.keys(bookingsRoot || {}), ...Object.keys(proposalsRoot || {})])) {
            if (bookingsRoot?.[candidateBoatId]?.[id]) {
                booking = bookingsRoot[candidateBoatId][id];
                locatedBoatId = candidateBoatId;
            }
            if (proposalsRoot?.[candidateBoatId]?.[id]) {
                proposal = proposalsRoot[candidateBoatId][id];
                locatedBoatId = locatedBoatId || candidateBoatId;
            }
        }
        ...
        if (!ownerId) throw new Error(`No Stripe owner is configured for boat ${boatId}`);
    ...
    async createOutingDepositCheckout(req: Request, res: Response) {
        try {
            ...
            await this.stbDbSvc.db.ref(this.buildBookingPaymentPath(boatId, bookingId, 'deposit')).set(payload);
        ...
        try {
            ...
            if (!booking) {
                ...
                for (const field of lookupFields) {
                    const snap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}`).orderByChild(field).equalTo(bookingId).once('value');
                    const matches = snap.val() || {};
            ...
            await paymentRef.set(payload);
            await this.stbDbSvc.db.ref(this.buildBookingPaymentPath(boatId, bookingId, 'warranty')).set(payload);
     ...
     */
    async completeOutingDepositPayment(req: Request, res: Response) {
        try {
            ...
            if (authorized) await this.markDepositAuthorizedFromStripe(params);
            else await this.markDepositPaidFromStripe(params);
    ...
    async completeOutingAdditionalPayment(req: Request, res: Response) {
        try {
            let { ownerId, bookingId, checkoutSessionId, sessionId } = req.body || {};
            checkoutSessionId = checkoutSessionId || sessionId;
            if (!bookingId) return res.status(400).json({ error: 'bookingId is required' });
            ...
            if (!paid) return res.status(409).json({ error: 'Stripe session is not paid yet' });

            const now = Date.now();
            const amountCents = Number(session.amount_total || paymentIntent?.amount_received || paymentIntent?.amount || 0);
            const skipperAmountCents = paymentType === 'skipper_fee' || paymentType === 'skipper'
                ? Number(metadata['skipperAmount'] || amountCents)
                : 0;
            const tipAmountCents = paymentType === 'skipper_fee' || paymentType === 'skipper'
                ? Number(metadata['tipAmount'] || 0)
                : 0;
            const tipBaseAmountCents = Number(metadata['tipBaseAmount'] || 0);
            const tipPercentage = metadata['tipPercentage'] === '' || metadata['tipPercentage'] === undefined
                ? null
                : Number(metadata['tipPercentage']);
            const tipMode = String(metadata['tipMode'] || '');
            ...
            const common = {
                ...
                skipperAmount: skipperAmountCents,
                skipperAmountEuros: skipperAmountCents / 100,
                tipAmount: tipAmountCents,
                tipAmountEuros: tipAmountCents / 100,
                tipBaseAmount: tipBaseAmountCents,
                tipBaseAmountEuros: tipBaseAmountCents / 100,
                tipPercentage,
                tipMode,
                currency: session.currency || paymentIntent?.currency || 'eur',
                checkoutSessionId: session.id,
            ...
            if (paymentType === 'skipper_fee' || paymentType === 'skipper') {
                updates[`/bnBookings/${boatId}/${bookingId}/payments/skipper`] = common;
                updates[`/bnBookings/${boatId}/${bookingId}/skipperPaid`] = true;
                updates[`/bnBookings/${boatId}/${bookingId}/skipperStatus`] = 'paid';
                updates[`/bnBookings/${boatId}/${bookingId}/skipperPaymentStatus`] = 'paid';
                updates[`/bnBookings/${boatId}/${bookingId}/skipperPaymentMethod`] = 'Stripe';
                updates[`/bnBookings/${boatId}/${bookingId}/skipperPaidAmount`] = skipperAmountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/tipAmount`] = tipAmountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/tipsAmount`] = tipAmountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/tipBaseAmount`] = tipBaseAmountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/tipPercentage`] = tipPercentage;
                updates[`/bnBookings/${boatId}/${bookingId}/tipMode`] = tipMode;
                updates[`/bnBookings/${boatId}/${bookingId}/skipperPaidAt`] = now;
            } else if (paymentType === 'extra_service' && itemId) {
                updates[`/bnBookings/${boatId}/${bookingId}/payments/extraServices/${itemId}`] = common;
            } else if (itemId) {
                updates[`/bnBookings/${boatId}/${bookingId}/payments/adHoc/${itemId}`] = common;
            } else {
                return res.status(400).json({ error: 'Additional payment item id is missing from Stripe metadata' });
            }
            updates[`/bnBookings/${boatId}/${bookingId}/modifiedTS`] = now;
            if (paymentId) updates[`/backendpayments/${paymentId}`] = common;
            await this.stbDbSvc.db.ref().update(updates);
    ...
    async completeOutingBalancePayment(req: Request, res: Response) {
        try {
            ...
            await Promise.all([
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/balance`).update(paymentPayload),
                paymentId ? this.stbDbSvc.db.ref(`/backendpayments/${paymentId}`).update(paymentPayload) : Promise.resolve(null),
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                    balanceStatus: 'paid',
                    balancePaid: true,
                    remainingPaid: true,
                ...
                this.stbDbSvc.db.ref(`/bnProposals/${boatId}/${bookingId}`).update({
                    balanceStatus: 'paid',
                    balancePaid: true,
                    remainingPaid: true,
                    remainingStatus: 'paid',
     ...
     * payment_method and stores it on /bnBookings/${boatId}/{bookingId}/payments/warranty.
     *
     * body: { ownerId?, bookingId, checkoutSessionId|sessionId }
     */
    async completeOutingWarrantySetup(req: Request, res: Response) {
        try {
            ...
            await Promise.all([
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update(cardPayload),
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/warranty`).update({
                    ...cardPayload,
                ...
                this.stbDbSvc.db.ref(`/bnProposals/${boatId}/${bookingId}`).update(cardPayload).catch(() => null),
            ]);
    ...
    async chargeOutingWarranty(req: Request, res: Response) {
        try {
            ...
            const warrantyPath = this.buildBookingPaymentPath(boatId, bookingId, 'warranty');
            const [warrantySnap, bookingSnap] = await Promise.all([
                this.stbDbSvc.db.ref(warrantyPath).once('value'),
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value'),
            ]);
            const booking = bookingSnap.val() || {};
            ...
            if (cashWarranty) {
                const chargeId = this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/warrantyCharges`).push().key || `damage_${Date.now()}`;
                const now = Date.now();
                ...
                const updates: Record<string, any> = {};
                updates[`/bnBookings/${boatId}/${bookingId}/payments/warrantyCharges/${chargeId}`] = chargeRecord;
                updates[`/bnBookings/${boatId}/${bookingId}/payments/warrantyCharge`] = {
                    ...chargeRecord,
                ...
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyStatus`] = totalChargedCents >= maxWarrantyCents && maxWarrantyCents > 0 ? 'fully_charged' : 'partially_charged';
                updates[`/bnBookings/${boatId}/${bookingId}/damageReported`] = true;
                updates[`/bnBookings/${boatId}/${bookingId}/damageCharged`] = true;
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargedAmount`] = totalChargedCents;
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargeAmount`] = amountCents;
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargeAmountCents`] = amountCents;
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargeAmountEuros`] = amountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargeStatus`] = 'paid';
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargeAt`] = now;
                updates[`/bnBookings/${boatId}/${bookingId}/lastWarrantyChargePaymentMethod`] = 'cash';
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargeAmountCents`] = amountCents;
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargeAmountEuros`] = amountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargeReason`] = reason;
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeAmount`] = amountCents / 100;
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeAmountCents`] = amountCents;
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeReason`] = reason;
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeStatus`] = 'paid';
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeError`] = null;
                updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeModifiedAt`] = now;
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargePaymentIntentId`] = null;
                updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargeRecordedAt`] = now;
                updates[`/bnBookings/${boatId}/${bookingId}/modifiedTS`] = now;
                updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargedAmount`] = totalChargedCents;
                updates[`/bnProposals/${boatId}/${bookingId}/lastWarrantyChargeAmount`] = amountCents;
                updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargeReason`] = reason;
                updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargeStatus`] = 'paid';
                updates[`/bnProposals/${boatId}/${bookingId}/modifiedTS`] = now;
            ...
            await this.stbDbSvc.db.ref().update({
                [`/bnBookings/${boatId}/${bookingId}/payments/warrantyCharges/${chargeId}`]: requestedChargeRecord,
                [`/bnBookings/${boatId}/${bookingId}/payments/warrantyCharge`]: {
                ...
                [`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeAmount`]: amountCents / 100,
                [`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeAmountCents`]: amountCents,
                [`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeReason`]: reason,
                [`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeStatus`]: 'processing',
                [`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeRequestedAt`]: now,
                [`/bnBookings/${boatId}/${bookingId}/modifiedTS`]: now,
            });
            ...
            updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeAmountCents`] = amountCents;
            updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeReason`] = reason;
            updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeStatus`] = chargeRecord.status;
            updates[`/bnBookings/${boatId}/${bookingId}/pendingWarrantyChargeModifiedAt`] = now;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargePaymentIntentId`] = pi.id;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyChargeRecordedAt`] = now;
            updates[`/bnBookings/${boatId}/${bookingId}/modifiedTS`] = now;
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargedAmount`] = totalChargedCents;
            updates[`/bnProposals/${boatId}/${bookingId}/lastWarrantyChargeAmount`] = amountCents;
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargeReason`] = reason;
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyChargeStatus`] = chargeRecord.status;
            updates[`/bnProposals/${boatId}/${bookingId}/modifiedTS`] = now;
            updates[`/backendpayments/${chargeId}`] = chargeRecord;
            ...
            try {
                ...
                if (failedBookingId && failedAmountCents > 0) {
                    const failedContext = await this.resolvePaymentContext(failedBookingId, (req.body || {}).ownerId).catch(() => null);
                    const failedBoatId = failedContext?.boatId || String((req.body || {}).boatId || 'alegria');
                    await this.stbDbSvc.db.ref(`/bnBookings/${failedBoatId}/${failedBookingId}`).update({
     ...
     */
    async releaseOutingWarranty(req: Request, res: Response) {
        try {
            ...
            const [bookingSnap, warrantySnap] = await Promise.all([
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value'),
                this.stbDbSvc.db.ref(this.buildBookingPaymentPath(boatId, bookingId, 'warranty')).once('value'),
            ]);
            const booking = bookingSnap.val() || {};
            if (!bookingSnap.exists()) return res.status(404).json({ error: 'Booking not found' });
            const warranty = warrantySnap.val() || {};
            ...
            const warrantyChargeItems: any[] = Object.keys(charges).map((key: string) => (charges as any)[key]);
            const successfulChargeCents: number = warrantyChargeItems.reduce((total: number, item: any): number => {
                const status = String(item?.status || '').toLowerCase();
                if (!successfulStatuses.has(status) && item?.paid !== true) return total;
            ...
            const chargedCents = Math.max(
                Number(booking.warrantyChargedAmount || 0),
                successfulChargeCents
            );
            ...
            ownerId = (await this.resolvePaymentContext(
                bookingId,
                warranty?.ownerId || ownerId
            )).ownerId;
            const warrantyMethod = String(
                warranty?.warrantyMethod || warranty?.warrantyPaymentChoice ||
                booking?.warrantyMethod || booking?.warrantyPaymentChoice || ''
            ...
            const now = Date.now();
            const releaseId = this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/warrantyReleases`).push().key || `release_${now}`;
            const releaseRecord = {
                releaseId,
                ownerId,
                bookingId,
            ...
            const updates: Record<string, any> = {};
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warrantyReleases/${releaseId}`] = releaseRecord;
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/status`] = 'released';
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/releasedAt`] = now;
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/releasedBy`] = releaseRecord.releasedBy;
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/noDamageObserved`] = true;
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/paymentMethodDetached`] = false;
            updates[`/bnBookings/${boatId}/${bookingId}/payments/warranty/canChargeWarranty`] = false;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyStatus`] = 'released';
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyReleased`] = true;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyReleasedAt`] = now;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyReleasedBy`] = releaseRecord.releasedBy;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyNoDamageObserved`] = true;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyActive`] = false;
            updates[`/bnBookings/${boatId}/${bookingId}/canChargeWarranty`] = false;
            updates[`/bnBookings/${boatId}/${bookingId}/warrantyPaymentMethodDetached`] = false;
            updates[`/bnBookings/${boatId}/${bookingId}/releasedWarrantyPaymentMethodId`] = paymentMethodId;
            updates[`/bnBookings/${boatId}/${bookingId}/releasedWarrantySetupIntentId`] = setupIntentId;
            updates[`/bnBookings/${boatId}/${bookingId}/modifiedTS`] = now;
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyStatus`] = 'released';
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyReleased`] = true;
            updates[`/bnProposals/${boatId}/${bookingId}/warrantyReleasedAt`] = now;
            updates[`/bnProposals/${boatId}/${bookingId}/canChargeWarranty`] = false;
            updates[`/bnProposals/${boatId}/${bookingId}/modifiedTS`] = now;
    ...
    async outingPaymentStatus(req: Request, res: Response) {
        try {
            const { bookingId, boatId: requestedBoatId } = req.query as { bookingId?: string; boatId?: string };
            if (!bookingId) return res.status(400).json({ error: 'bookingId is required' });
            const { boatId } = await this.resolvePaymentContext(bookingId, undefined);

            const snap = await this.stbDbSvc.db.ref(this.buildBookingPaymentPath(boatId, bookingId)).once('value');
    ...
    private async markDepositAuthorizedFromStripe(params: {
        ...
        const paymentContext = await this.resolvePaymentContext(params.bookingId, params.ownerId);
        const boatId = paymentContext.boatId;
        const now = Date.now();
        const updatePayload = {
            status: 'authorized',
        ...
        await Promise.all([
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payments/deposit`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payment`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payments/deposit`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}`).update({
                depositStatus: 'authorized',
                depositAuthorized: true,
                depositPaid: false,
    ...
    private async markDepositPaidFromStripe(params: {
        ...
        const bookingSnap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}`).once('value').catch(() => null);
        const offerSnap = await this.stbDbSvc.db.ref(`/bnProposals/${boatId}/${params.bookingId}`).once('value').catch(() => null);
        ...
        await Promise.all([
            ...
            this.stbDbSvc.db.ref(`/bnProposals/${boatId}/${params.bookingId}`).update({
                depositStatus: 'paid',
                depositPaid: true,
                depositPaidAmount: params.amount ? params.amount / 100 : 0,
                paidDepositAmount: params.amount ? params.amount / 100 : 0,
    ...
    private async markWarrantySavedFromStripe(params: {
        ...
        await Promise.all([
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payments/warranty`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payment`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}/payments/warranty`).update(updatePayload),
            this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${params.bookingId}`).update({
                warrantyStatus: 'card_registered',
    ...
    async handleOwnerWebhook(req: Request, res: Response) {
        let ownerId = String((req.params as any)?.ownerId || '');

        let event: Stripe.Event;
        try {
            const preverifiedEvent = (req as any).stripeEvent as Stripe.Event | undefined;
            if (preverifiedEvent) {
        ...
        if (!ownerId && event.account) {
            const owners = await this.stbDbSvc.db.ref('/backendowners').once('value').then(s => s.val()).catch(() => ({}));
            ownerId = Object.keys(owners || {}).find(id => owners?.[id]?.stripeConnect?.accountId === event.account) || '';
        }
        if (!ownerId) return res.status(400).send('Unable to resolve connected Stripe owner');
        const eventObject: any = event.data?.object || {};
        let boatId = String(eventObject?.metadata?.boatId || '');
        if (!boatId) {
            const fleet = await this.stbDbSvc.db.ref('/bnFleet').once('value').then(s => s.val()).catch(() => ({}));
            boatId = Object.keys(fleet || {}).find(id => String(fleet?.[id]?.ownerId || '') === ownerId) || ownerId;
        }
            ...
            switch (event.type) {
                ...
                case 'checkout.session.completed': {
                    ...
                    if (session.mode === 'setup' && session.setup_intent) {
                        ...
                        await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/warranty`).update(updatePayload);
                        await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/warranty`).update(updatePayload);
                        await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                            warrantyStatus: 'card_registered',
                            warrantyRegistered: true,
                    ...
                    if (session.mode === 'payment' && session.payment_intent) {
                        ...
                        if (isBalancePayment) {
                            ...
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/balance`).update({
                                ...updatePayload,
                                paid: true,
                                paymentStatus: 'paid',
                                paymentType: 'balance',
                            ...
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/alegria`).update({
                                ...updatePayload,
                                paid: true,
                                paymentStatus: 'paid',
                            ...
                            const paidAmountEuros = Number(session.amount_total || 0) / 100;
                            const skipperAmountEuros = Number(session.metadata?.['skipperAmount'] || session.amount_total || 0) / 100;
                            const tipAmountEuros = Number(session.metadata?.['tipAmount'] || 0) / 100;
                            const tipBaseAmountEuros = Number(session.metadata?.['tipBaseAmount'] || 0) / 100;
                            const tipPercentage = session.metadata?.['tipPercentage'] === '' || session.metadata?.['tipPercentage'] === undefined
                                ? null
                                : Number(session.metadata?.['tipPercentage']);
                            const tipMode = String(session.metadata?.['tipMode'] || '');
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/skipper`).update({
                                ...updatePayload,
                                paid: true,
                                paymentStatus: 'paid',
                            ...
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                                ...
                                skipperPaidAmount: skipperAmountEuros,
                                tipAmount: tipAmountEuros,
                                tipsAmount: tipAmountEuros,
                                tipBaseAmount: tipBaseAmountEuros,
                                tipPercentage,
                                tipMode,
                                skipperPaidAt: now,
                                modifiedTS: now,
                            ...
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/extraServices`).set(updatedExtraServices);
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/extraServices/${extraServiceId}`).update(updatePayload);
                            ...
                            if (adhocPaymentId) {
                                await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/adHoc/${adhocPaymentId}`).update(updatePayload);
                            }
                        } else {
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/deposit`).update({ ...updatePayload, status: 'deposit_paid' });
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/deposit`).update({ ...updatePayload, status: 'deposit_paid' });
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                                depositStatus: 'paid',
                                depositPaid: true,
                ...
                case 'payment_intent.succeeded': {
                    ...
                    if (bookingId) {
                        ...
                        if (isBalance) {
                            ...
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/balance`).update({ ...common, paymentType: 'balance' });
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/alegria`).update({ ...common, paymentType: 'alegria_balance' });
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                            ...
                            const skipperAmountEuros = Number(metadata['skipperAmount'] || pi.amount_received || pi.amount || 0) / 100;
                            const tipAmountEuros = Number(metadata['tipAmount'] || 0) / 100;
                            const tipBaseAmountEuros = Number(metadata['tipBaseAmount'] || 0) / 100;
                            const tipPercentage = metadata['tipPercentage'] === '' || metadata['tipPercentage'] === undefined
                                ? null
                                : Number(metadata['tipPercentage']);
                            const tipMode = String(metadata['tipMode'] || '');
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/skipper`).update({
                                ...
                                amountEuros, skipperAmountEuros, tipAmountEuros, tipBaseAmountEuros, tipPercentage, tipMode,
                                currency: pi.currency || 'eur', paidAt: now, modifiedTS: now, updatedAt: now,
                            });
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                                skipperPaid: true, skipperStatus: 'paid', skipperPaymentStatus: 'paid', skipperPaymentMethod: 'Stripe',
                                skipperPaidAmount: skipperAmountEuros, tipAmount: tipAmountEuros, tipsAmount: tipAmountEuros,
                                tipBaseAmount: tipBaseAmountEuros, tipPercentage, tipMode,
                                skipperPaidAt: now, modifiedTS: now, updatedAt: now,
                            });
                        } else {
                            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update({
                                paymentPaymentIntentId: pi.id, modifiedTS: now, updatedAt: now,
    ...
    async handlePlatformWebhook(req: Request, res: Response, endpointSecret: string) {
        try {
            ...
            (req as any).params = { ...((req as any).params || {}), ownerId: 'alegria' };
    ...
    async acceptOutingBookingRequest(req: Request, res: Response) {
        try {
            ...
            const snap = await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).once('value');
            ...
            await Promise.all([
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update(updatePayload),
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/deposit`).update({
                    status: 'captured',
                    depositStatus: 'paid',
                    depositPaid: true,
                ...
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`).update(updatePayload).catch(() => undefined),
                this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/deposit`).update({
                    status: 'captured',
                    depositStatus: 'paid',
                    depositPaid: true,
   ...
   */
    async ownerStripeStatus(req: Request, res: Response) {
        try {
            ...
            const path = `/backendowners/${ownerId}/stripeConnect`;
            const data = await this.stbDbSvc.db.ref(path).once('value').then(s => s.val()).catch(() => null);
            if (!data) {
                // Not connected
                return res.json({
                    connected: false,
            ...
            const account = await PLATFORM.accounts.retrieve(String(data.accountId));
            if ((account as any).deleted) throw new Error('Connected Stripe account was deleted.');
            const safe = {
                accountId: account.id,
                detailsSubmitted: !!account.details_submitted,
                chargesEnabled: !!account.charges_enabled,
            ...
            await this.stbDbSvc.db.ref(path).update(safe);
            return res.json({
                connected: safe.detailsSubmitted && safe.chargesEnabled,
                ...safe,
    ...
    setRoutes(stripeRouter: any) {
        ...
        stripeRouter.post('/pay/outing-skipper-fee-checkout', (req: Request, res: Response) =>
            this.createOutingExtraServiceCheckout(req, res)
        );
        stripeRouter.post('/api/payments/create-skipper-fee-checkout-session', (req: Request, res: Response) =>
            this.createOutingExtraServiceCheckout(req, res)
        );
        stripeRouter.post('/stripe/skipper-fee-checkout', (req: Request, res: Response) =>
            this.createOutingExtraServiceCheckout(req, res)
        );
        stripeRouter.post('/pay/outing-terms-accept', (req: Request, res: Response) =>
            this.acceptOutingTerms(req, res)
        );
        stripeRouter.post('/api/bookings/:bookingId/accept-terms', (req: Request, res: Response) =>
            this.acceptOutingTerms(req, res)
        );
        ...
        stripeRouter.post('/pay/outing-deposit-complete', (req: Request, res: Response) =>
            this.completeOutingDepositPayment(req, res)
        );
        stripeRouter.post('/api/payments/complete-deposit-payment', (req: Request, res: Response) =>
            this.completeOutingDepositPayment(req, res)
        );
        stripeRouter.post('/stripe/deposit-complete', (req: Request, res: Response) =>
            this.completeOutingDepositPayment(req, res)
        );
        stripeRouter.post('/pay/outing-additional-payment-complete', (req: Request, res: Response) =>
            this.completeOutingAdditionalPayment(req, res)
        );
        stripeRouter.post('/api/payments/complete-additional-payment', (req: Request, res: Response) =>
            this.completeOutingAdditionalPayment(req, res)
        );
        stripeRouter.post('/stripe/additional-payment-complete', (req: Request, res: Response) =>
            this.completeOutingAdditionalPayment(req, res)
        );
        ...
        stripeRouter.post('/api/payments/release-warranty', (req: Request, res: Response) =>
            this.releaseOutingWarranty(req, res)
        );
        ...
        stripeRouter.post('/pay/outing-warranty-release', (req: Request, res: Response) =>
            this.releaseOutingWarranty(req, res)
        );
        stripeRouter.post('/stripe/warranty-release', (req: Request, res: Response) =>
            this.releaseOutingWarranty(req, res)
        );
    ...
    async createOutingBalanceCheckout(req: any, res: any) {
        try {
            ...
            await paymentRef.set(payload);
            await this.stbDbSvc.db.ref(this.buildBookingPaymentPath(boatId, bookingId, 'balance')).set(payload);
            await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/balance`).set(payload);
    ...
    async createOutingExtraServiceCheckout(req: any, res: any) {
        try {
            ...
            if (bookingId) {
                await this.assertOrPersistCheckoutTermsAcceptance(bookingId, body);
            }
            ...
            const skipperAmountCents = isSkipperFeePayment
                ? this.normalizeAmountToCents(body.skipperAmount ?? rawAmount)
                : 0;
            const tipAmountCents = isSkipperFeePayment
                ? Math.max(0, this.normalizeAmountToCents(body.tipAmount))
                : 0;
            const tipBaseAmountCents = isSkipperFeePayment
                ? Math.max(0, this.normalizeAmountToCents(body.tipBaseAmount))
                : 0;
            const tipPercentage = isSkipperFeePayment && body.tipPercentage !== null && body.tipPercentage !== undefined
                ? Math.max(0, Number(body.tipPercentage) || 0)
                : null;
            const tipMode = isSkipperFeePayment ? String(body.tipMode || (tipPercentage === null ? 'custom' : 'percentage')) : '';
            if (isSkipperFeePayment && Math.abs(amount - (skipperAmountCents + tipAmountCents)) > 1) {
                return res.status(400).json({
                    error: 'Skipper payment total must equal skipperAmount plus tipAmount',
                    received: { amount, skipperAmount: skipperAmountCents, tipAmount: tipAmountCents }
                });
            }
            ...
            const session = await stripe.checkout.sessions.create({
                ...
                payment_intent_data: {
                    metadata: {
                        ...
                        adhocPaymentId: isAdHocPayment ? extraServiceId : '',
                        description,
                        skipperAmount: String(skipperAmountCents),
                        tipAmount: String(tipAmountCents),
                        tipBaseAmount: String(tipBaseAmountCents),
                        tipPercentage: tipPercentage === null ? '' : String(tipPercentage),
                        tipMode,
                    },
                },
                metadata: {
            ...
            if (isSkipperFeePayment && bookingId) {
                await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/skipper`).set(payload);
            } else if (isAdHocPayment && bookingId) {
                await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/adHoc/${extraServiceId}`).set(payload);
                ...
                await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/payments/extraServices/${extraServiceId}`).set(payload);
                ...
                await this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}/extraServices`).set(extraServices);
            }
    ...
    async refundOutingPayment(req: any, res: any) {
        try {
            ...
            const bookingRef = this.stbDbSvc.db.ref(`/bnBookings/${boatId}/${bookingId}`);
            ...
            if (paymentType === 'deposit') {
                paymentPath = `/bnBookings/${boatId}/${bookingId}/payments/deposit`;
            } else if (paymentType === 'balance') {
                paymentPath = `/bnBookings/${boatId}/${bookingId}/payments/balance`;
            } else if (paymentType === 'skipper_fee') {
                paymentPath = `/bnBookings/${boatId}/${bookingId}/payments/skipper`;
            } else if (paymentType === 'extra_service') {
                if (extraServiceId) {
                    paymentPath = `/bnBookings/${boatId}/${bookingId}/payments/extraServices/${extraServiceId}`;
                } else {
                    itemCollectionPath = `/bnBookings/${boatId}/${bookingId}/payments/extraServices`;
                }
            } else if (paymentType === 'ad_hoc') {
                if (extraServiceId) {
                    paymentPath = `/bnBookings/${boatId}/${bookingId}/payments/adHoc/${extraServiceId}`;
                } else {
                    itemCollectionPath = `/bnBookings/${boatId}/${bookingId}/payments/adHoc`;
                }
            } else {
                return res.status(400).json({
            ...
            if (itemCollectionPath) {
                ...
                const matching = entries.filter((entry: any) =>
                    (explicitPaymentIntentId && (entry.value.stripePaymentIntentId === explicitPaymentIntentId || entry.value.paymentIntentId === explicitPaymentIntentId)) ||
                ...
                const refundable = entries.filter((entry: any) => {
                ...
                if (!selected) {
                    return res.status(400).json({
                        ...
                        bookingId,
                        paymentType,
                        availablePaymentIds: entries.map((entry: any) => entry.id)
                    });
                }
