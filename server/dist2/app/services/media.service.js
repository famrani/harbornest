...
class MediaService {
    constructor(store) {
        ...
        this.supportedResourceTypes = new Set(['boat', 'place', 'car', 'pool']);
    ...
    isAllowedPath(objectPath, tenant) {
        ...
        if (tenant.mediaPrefix && path.startsWith(tenant.mediaPrefix))
            return true;
    ...
    normalizeTenantConfigs(raw) {
        const normalized = {};
        if (!raw || typeof raw !== 'object')
            return normalized;
        const looksLikeTenant = (value) => !!value && typeof value === 'object'
