...
async function loadEmailTemplate(db: any, lang: string, key: string, boatId = 'alegria'): Promise<EmailTemplate> {
...
export class BookingsService {
    ...
    private async locateRecordPath(collection: 'bnBookings' | 'bnProposals', id: string, preferredBoatId?: string): Promise<string> {
        const preferred = String(preferredBoatId || '').trim();
        if (preferred) {
            const path = `/${collection}/${preferred}/${id}`;
            if (await this.storeDbc.db.ref(path).once('value').then(s => s.exists()).catch(() => false)) return path;
        }
        const root = await this.storeDbc.db.ref(`/${collection}`).once('value').then(s => s.val()).catch(() => ({}));
        for (const boatId of Object.keys(root || {})) {
            if (root?.[boatId]?.[id]) return `/${collection}/${boatId}/${id}`;
        }
        return `/${collection}/${preferred || 'alegria'}/${id}`;
    }
    ...
    async indexBookingOnCalendar(booking: any): Promise<void> {
        ...
        for (const day of days) {
            updates[`/backendcalendar/${booking.boatId || 'alegria'}/${day}/${bookingId}`] = {
                ...
                guestRef: this.calendarCustomerRef(
    ...
    async unindexBookingFromCalendar(bookingId: string, startISO: string, endISO: string): Promise<void> {
        const bookingPath = await this.locateRecordPath('bnBookings', bookingId);
        const bookingSnap = await this.storeDbc.db.ref(bookingPath).once('value');
    ...
    async createBooking(raw: any): Promise<{ bookingId: string; booking: Booking }> {
        ...
        const ref = this.storeDbc.db.ref(`${OBJECTNAME.bnBookings}/${boatId}`).push();
        const bookingId = ref.key as string;
        const boat = await this.storeDbc.db.ref(`${OBJECTNAME.bnFleet}/${boatId}`).once('value')
        ...
        const ownerStripeData = await this.storeDbc.db.ref(`/backendowners/${raw.ownerId}/stripeConnect`)
            .once('value')
            .then(s => s.val());

        const stripeReady = !!ownerStripeData?.accountId && ownerStripeData?.chargesEnabled === true;
        const booking: Booking = {
                ...
                payment: {
                    ...
                    stripe_user_id: ownerStripeData?.accountId || null,
            ...
            const updates: any = {};
            updates[`${OBJECTNAME.bnBookings}/${boatId}/${bookingId}`] = booking;
    ...
    async updateBookingStatus(
        ...
        const bookingPath = await this.locateRecordPath('bnBookings', bookingId);
        const snap = await this.storeDbc.db.ref(bookingPath).once('value');
        ...
        const updates: any = {};
        updates[`${bookingPath}/status`] = newStatus;
        updates[`${bookingPath}/statusReason`] = reason || null;
        updates[`${bookingPath}/updatedAt`] = now;
        updates[`${bookingPath}/statusHistory/${now}`] = {
    ...
    private calendarCustomerRef(value: any): string | null {
    ...
    async getGuestInfo(boatId = 'alegria') {
        const snap = await this.storeDbc.db.ref(`${OBJECTNAME.guestInfo}/${boatId}`).once('value');
        return snap.val() || {};
    }

    async getExtraServicesCatalog(boatId = 'alegria') {
        const snap = await this.storeDbc.db.ref(`${OBJECTNAME.bnFleet}/${boatId}/extraServices`).once('value');
    ...
    private async notifyCustomerRequestChange(req: Request, kind: 'updated' | 'cancelled', source: 'booking' | 'offer', id: string, patchPayload: any = {}): Promise<void> {
        const path = await this.locateRecordPath(source === 'booking' ? 'bnBookings' : 'bnProposals', id, patchPayload?.boatId);
        ...
        const boatId = String(record?.boatId || patchPayload?.boatId || 'alegria');
        const boatBaseUrl = `${origin}/${encodeURIComponent(boatId)}`;
        const adminUrl = source === 'booking' ? `${boatBaseUrl}/admin/bookings/${encodeURIComponent(id)}` : `${boatBaseUrl}/admin/offers`;
        const customerUrl = `${boatBaseUrl}/my-offers`;
        ...
        const adminTemplate = await loadEmailTemplate(this.storeDbc.db, lang, adminKey, boatId);
        ...
        if (email) {
            const customerTemplate = await loadEmailTemplate(this.storeDbc.db, lang, customerKey, boatId);
    ...
    private async notifyBookingRequestCreated(req: Request, offerId: string): Promise<void> {
        const proposalPath = await this.locateRecordPath('bnProposals', offerId);
        const snap = await this.storeDbc.db.ref(proposalPath).once('value');
        ...
        const origin = getPublicAppOrigin(req);
        const boatId = String(offer?.boatId || 'alegria');
        const boatBaseUrl = `${origin}/${encodeURIComponent(boatId)}`;
        const offerUrl = `${boatBaseUrl}/admin/offers`;
        const customerOfferUrl = `${boatBaseUrl}/my-offers`;
        ...
        const adminTemplate = await loadEmailTemplate(this.storeDbc.db, lang, 'bookingRequestAdmin', boatId);
        ...
        let customerSent = false;
        if (email) {
            const customerTemplate = await loadEmailTemplate(this.storeDbc.db, lang, 'bookingRequestCustomer', boatId);
        ...
        await this.storeDbc.db.ref(proposalPath).update({
    ...
    private async notifyOfferReady(req: Request, offerId: string): Promise<void> {
        ...
        const origin = getPublicAppOrigin(req);
        const boatId = String(offer?.boatId || 'alegria');
        const offerUrl = `${origin}/${encodeURIComponent(boatId)}/offer/${encodeURIComponent(offerId)}`;
        const template = await loadEmailTemplate(this.storeDbc.db, lang, 'offerReady', boatId);
        const finance = offerFinancialBreakdown(offer);
        const data = {
            customerName: name || '',
            offerId,
    ...
    private async notifyBookingConfirmed(req: Request, bookingId: string): Promise<void> {
        ...
        const origin = getPublicAppOrigin(req);
        const boatId = String(booking?.boatId || 'alegria');
        const bookingUrl = `${origin}/${encodeURIComponent(boatId)}/bookings/${encodeURIComponent(bookingId)}`;
        const template = await loadEmailTemplate(this.storeDbc.db, lang, 'bookingConfirmed', boatId);
        ...
        await this.mailer.sendToGuest(email, subject, html);
        await this.storeDbc.db.ref(bookingPath).update({
        ...
        const notificationHandler = async (req: Request, res: Response) => {
            try {
                ...
                const eventBase = payload.bookingId
                    ? `${await this.locateRecordPath('bnBookings', payload.bookingId, (payload as any).boatId)}/events/${eventId}`
                    : payload.offerId
                        ? `${await this.locateRecordPath('bnProposals', payload.offerId, (payload as any).boatId)}/events/${eventId}`
                        : `/bnFleet/${(payload as any).boatId || 'alegria'}/events/${eventId}`;
                ...
                if (eventId) {
                    const failedBase = req.body?.bookingId
                        ? `${await this.locateRecordPath('bnBookings', req.body.bookingId, req.body?.boatId)}/events/${eventId}`
                        : req.body?.offerId
                            ? `${await this.locateRecordPath('bnProposals', req.body.offerId, req.body?.boatId)}/events/${eventId}`
