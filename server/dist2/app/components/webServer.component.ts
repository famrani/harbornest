...
export class WebServerComponent {
    ...
    async initWebServer(): Promise<void> {
        ...
        // One trusted reverse proxy between the internet and Express.
    ...
    private setupMiddlewares(): void {
        ...
        this.app.post('/stripe/owner/:ownerId/webhook', bodyParser.raw({ type: 'application/json' }), (req: Request, res: Response) =>
            this.stripeSvc.handleOwnerWebhook(req as any, res)
        );
        this.app.post('/stripe/connect/webhook', bodyParser.raw({ type: 'application/json' }), (req: Request, res: Response) =>
            this.stripeSvc.handleOwnerWebhook(req as any, res)
        );
        this.app.post('/stripe/webhook', bodyParser.raw({ type: 'application/json' }), (req: Request, res: Response) =>
            this.stripeSvc.handlePlatformWebhook(req as any, res, process.env.STRIPE_PLATFORM_WEBHOOK_SECRET || process.env.STRIPE_WEBHOOK_SECRET || '')
        );
    ...
    private setRoutes(): void {
        ...
        this.mediaSvc.setRoutes(this.router);
    }
}
